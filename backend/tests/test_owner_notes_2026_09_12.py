...
def kpr_contract(sa):
    ...
    rows = [c for c in r.json()["data"] if not (c.get("legal") or {}).get("akad_kredit")]
    assert rows, "tidak ada kontrak KPR demo tanpa akad"
    c = rows[0]
    org = c["org_id"]
    ...
    _db.settings.update_one({"key": "addon.require_spkt_for_excess_land", "org_id": org},
                            {"$set": {"value": False}}, upsert=True)
...
def test_akad_gate_overridable_and_setting(sa, kpr_contract):
    org = kpr_contract["org_id"]
    _db.settings.update_one({"key": "addon.excess_land_must_be_paid_before_akad", "org_id": org},
