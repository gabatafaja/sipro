...
async def simulate(org: str = ORG_ID, *, unit_id: str, addons: list = None,
    ...
    max_pct = float(disc["approval_thresholds"]["max_pct"])
    ...
    calc = {
        ...
        "needs_discount_approval": disc["needs_approval"],
        "approval_reasons": disc["approval_reasons"],
        "approval_thresholds": disc["approval_thresholds"],
...
def refresh_totals(calc: dict) -> dict:
    ...
    # Persetujuan dihitung ULANG di sini karena potongan komponen biaya baru bernilai setelah
    # skema all-in menempel — ambang nominal harus melihat angka akhirnya.
    th = calc.get("approval_thresholds") or {}
    appr = pe.evaluate_approval(lines, int(calc.get("gross_price") or 0),
                                max_pct=th.get("max_pct", calc.get("discount_limit_pct") or 0),
                                min_amount=th.get("min_amount", 0))
    calc["needs_discount_approval"] = appr["needs_approval"]
    calc["approval_reasons"] = appr["reasons"]
...
async def create(org: str = ORG_ID, *, lead_id: str, unit_id: str, addons: list = None,
    ...
    doc = {
        ...
        "approval_reasons": calc.get("approval_reasons") or [],
    ...
    if state == "awaiting_approval":
        reasons = "; ".join(r.get("label") or "" for r in doc.get("approval_reasons") or []) or (
            f"diskon melebihi batas {doc['discount_limit_pct']}%")
        for mgr in await approver_emails(org):
            await create_notification(
                user_email=mgr, org_id=org, type="sales",
                title="Persetujuan diskon penawaran",
                body=(f"{doc['no']} unit {doc['unit_code']}: potongan Rp "
                      f"{doc['discount_amount']:,} ({doc['discount_pct']}%) — {reasons}").replace(",", "."),
...
async def approver_emails(org: str) -> list:
    """Semua pengguna aktif yang memegang izin `pricing:approve` (peran kustom ikut)."""
    from rbac import can
    out = []
    async for u in db.users.find({"org_id": org, "is_active": {"$ne": False}},
                                 {"_id": 0, "email": 1, "role": 1}):
        if u.get("email") and await can(u.get("role"), "pricing", "approve"):
            out.append(u["email"])
    return out[:30]
...
def pricing_snapshot(calc: dict) -> dict:
    ...
    keys = ("base_price", "addon_total", "addon_net_total", "addon_discount_amount",
            ...
            "needs_discount_approval", "approval_reasons",
