...
APPROVAL_MODES = ("global", "always", "never")
...
class PricingRuleBase(BaseModel):
    ...
    # Persetujuan manajer — berlaku untuk KETIGA skema (diskon, promo, kupon):
    #   global = ikut ambang organisasi (persen & nominal), always = selalu, never = tidak pernah.
    approval_mode: str = "global"
    ...
    @classmethod
    def _approval_mode(cls, v):
        if v not in APPROVAL_MODES:
            raise ValueError("Mode persetujuan harus global/always/never.")
        return v

    @model_validator(mode="after")
    def _sync_requires(self):
        if self.requires_approval and self.approval_mode == "global":
            self.approval_mode = "always"
        self.requires_approval = self.approval_mode == "always"
        return self
